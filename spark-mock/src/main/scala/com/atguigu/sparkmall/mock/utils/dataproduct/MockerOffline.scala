package com.atguigu.sparkmall.mock.utils.dataproduct

import java.text.SimpleDateFormat
import java.util.UUID

import com.atguigu.sparkmall.common.bean.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.atguigu.sparkmall.mock.utils.base.{RanOpt, RandomDate, RandomNum, RandomOptions}
...
object MockerOffline {
  ...
  def insertHive(sparkSession: SparkSession, tableName: String, dataFrame: DataFrame): Unit = {
    ...
    println(s"向${tableName}表中保存数据已完成!")
    sparkSession.sql("select * from " + tableName).show(10)
